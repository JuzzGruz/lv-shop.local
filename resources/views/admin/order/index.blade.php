@extends('layouts.adminPanel', ['title' => 'Все заказы'])

@section('content')
    @if (!empty(session('notFound')))
        <div class="alert alert-danger col-3">
            <ul class="m-0">
                @foreach (session('notFound') as $key => $value)
                    <li class="list-unstyled fw-bold fs-6">Товар {{ $key }} в кол-ве {{ $value }}, не найден в базе данных</li>
                @endforeach
            </ul>
        </div>
    @endif
    @if (!empty(session('prodQuantity')))
        <div class="alert alert-success alert-dismissible mt-4">
            <ul class="m-0">
                @foreach (session('prodQuantity') as $key => $value)
                    <li class="list-unstyled fw-bold fs-6">Товару {{ $key }} вернулось в количество {{ $value }} ед. товара</li>
                @endforeach
            </ul>
        </div>
    @endif
